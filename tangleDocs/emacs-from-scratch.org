...
#+DESCRIPTION: This is my attempt at creating my own emacs config from the ground up without doom or other starter kits.
#+STARTUP: overview num hidestars indent
#+PROPERTY: header-args :results silent :tangle "../configs/.scratch.d/config.el"
...
Unlike other literate configs, this one is not meant to be compiled at start time. This gets tangled together whenever it is saved and then can be read into emacs by restarting emacs.
...
- [[#old-settings][Old settings]]
  ...
  - [[#fixes][Fixes]]
    ...
    - [[#double-percent-sign-in-modeline-battery-reporting][Double percent sign in modeline battery reporting]]
...
#+begin_src emacs-lisp :tangle ../.scratch.d/init.el
...
#+begin_src emacs-lisp :tangle ../.scratch.d/packages.el
;; -*- no-byte-compile: t; -*-
;;; $DOOMDIR/packages.el
#+end_src
...
#+begin_src elisp :tangle ../.scratch.d/packages.el
(package! diff-ansi)
#+end_src
...
*** Double percent sign in modeline battery reporting
Fix double percent sign.
Pinned at a version just before the mode line transitioned to nerd fonts since nerd fonts don't seem to work well and don't seem to be generally integrated with doom emacs.
#+begin_src emacs-lisp :tangle ../.scratch.d/packages.el
