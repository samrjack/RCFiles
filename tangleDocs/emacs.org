...
The standard folding capability of emacs only will fold when the cursor is over the thing that is folding.
This function makes folds occur from anywhere on the line.
#+begin_src emacs-lisp
...
Then remap the evil fold functions that need it to use the above function.
#+begin_src emacs-lisp
...
Lets make sure the ts-fold package is installed so that we can use folding.
#+begin_src emacs-lisp :tangle ../configs/.doom.d/packages.el
...
Finally, set go-mode to use ts-fold by default since it gives such good results.
#+begin_src emacs-lisp
