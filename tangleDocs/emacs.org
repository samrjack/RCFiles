...
*** Git
**** Blamer
This module adds git commit information in line with code when turned on whcih can be nice when working through a code base.
#+BEGIN_SRC elisp :tangle ../configs/.doom.d/packages.el
(package! blamer)
#+END_SRC
...
***** Org template
All of my org templates are in a seprate folder in my org files repo. This is so that templates are both private from public eyes and also travel with their partnered org files. I'll probably make a seporate git repo for them just so they're recorded but that's a lower priority.
...
This block isn't tangled and isn't at all guarenteed to work even if it was so use it only as an example.
#+BEGIN_SRC elisp :tangle no
(after! org
