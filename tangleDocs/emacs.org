...
Although these are technically "after org loads", they are easier to set using the built in key mapping functionality so I'm placing them here instead.
#+BEGIN_SRC elisp
...
**** After org load
***** Start of "after! org"
These configurations must all be made after org is loaded so the must be placed int the =after!= macro.
#+BEGIN_SRC elisp
(after! org
#+END_SRC
...
VLF if a package for lazy loading enormous files that would normally take forever to load. However, I've ran into issues with using it and therefore I'm just leaving it off for the time being.
#+BEGIN_SRC elisp :tangle no
